...
return: RETURN SEMICOLON { string instruction = "return"; gencode(instruction);} | RETURN id SEMICOLON { string instruction = "return " + string($2); gencode(instruction);} | RETURN int_constant SEMICOLON { string instruction = "return " + string($2); gencode(instruction);} | RETURN arithmetic_exp SEMICOLON { string instruction = "return " + print_element(icg_stack.top()); gencode(instruction);}
...
void gencode(string s) {
    ...
    insn = to_string(TAC_lineno++) + string(": ");
    while(insn.length() < 6) {
        insn = insn + " ";
    }
    insn = insn + s;
